class CartsController < ApplicationController
  ...
  def remove_product
    product_id = params[:product_id].to_s
    cart.delete(product_id)  # Directly delete the product from the cart
    update_cart(cart)        # Update the cart session after removal
    redirect_back(fallback_location: root_path) # Redirect to the previous page
  end
